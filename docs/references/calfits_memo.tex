...
\begin{enumerate}
	...
	These arrays both have the shape (Nants\_data, Nspws, Nfreqs, Ntimes, Njones), and are both flattened before concatenation, so the length of this axis will be $2\times$(Nants\_data + Nspws + Nfreqs + Ntimes + Njones), where these variables are those stored in the corresponding UVCal object.
	...
	These arrays all have the shape (Nants\_data, Nspws, Nfreqs, Ntimes, Njones), and are all flattened before concatenation, so the length of this axis will be $3\times$(Nants\_data + Nspws + Nfreqs + Ntimes + Njones), or $4\times$(Nants\_data + Nspws + Nfreqs + Ntimes + Njones), depending on whether there is an input\_flag\_array, and where these variables are those stored in the corresponding UVCal object.
	\end{itemize}
