...
\begin{enumerate}
	...
	In older files, the quality\_array is always present. In newer files it can be absent but a boolean header item \textbf{HASQLTY} will be present to
	make it clear which array is present in the even that the axis is length 4. So if the axis is length 4 and there is no \textbf{HASQLTY} header keyword,
