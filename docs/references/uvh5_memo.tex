...
\item \textbf{flex\_spw\_id\_array}: \textit{int} The mapping of individual
  ...
  (Nfreqs). (\textit{flex\_spw\_id\_array})
...
\item \textbf{flex\_spw}: \textit{numpy bool}\footnote{Note that this is
    ...
    \texttt{FALSE} value. See Appendix~\ref{appendix:boolean} for an example of
    how to define this in C. Such a definition should follow analogously in
    other languages.}  Whether the data are saved using flexible spectral
  windows. Following the convention above, this should always be
  \verb+True+. See Sec.~\ref{sec:flex_spw} for a discussion of historical files
  for which this was not true. (\textit{flex\_spw})
...
spectral windows. Initially, inspired by the handling of multiple spectral
...
\item The \verb+flex_spw_id_array+ dataset was added to identify which spectral
  window a given channel belongs.
...
Although both kinds of datasets (with or without flexible spectral windows) are
supported by the UVH5 specification, it is preferred that they be saved
\textit{with} flexible spectral windows, even in the case where there is a
single spectral window. This helps standardize the UVH5 format going forward,
while being backwards compatible with legacy files.
...
previously. Multiple issues changed in a single revision are offset using double
horizontal lines.
...
\begin{center}
  \begin{tabular}{ m{8em} | m{10em} | m{10em} | m{4em} }
    ...
    \texttt{Header/version} & String corresponding to version & Not present & v0.1 \\[1em] \hline\hline
    \makecell[tl]{\texttt{Header/}\\\texttt{integration\_time}} & Array of float, shape (Nblts) & Single float (assumed to apply to all baseline-times) & v0.1 \\[1.75em] \hline\hline
    \makecell[cl]{\texttt{Header/}\\\texttt{channel\_width}} & Array of float, shape (Nfreqs) & Single float (assumed to apply to all frequencies) & v0.1 \\[1.75em] \hline
    \makecell[cl]{\texttt{Header/}\\\texttt{flex\_spw\_id\_array}} & Array of int, shape (Nfreqs) & Not present & v0.1 \\[1em] \hline
    \texttt{Header/flex\_spw} & Numpy bool, value of \texttt{True} & Not present & v0.1 \\[1em] \hline\hline
    \makecell[cl]{\texttt{Header/}\\\texttt{freq\_array}} & Array of float, shape (Nfreqs) & Array of float, shape (Nspws, Nfreqs) & v1.0\\[1em] \hline
    \texttt{Data/visdata} & Array of complex, shape (Nblts, Nfreqs, Npols) & Array of complex, shape (Nblts, Nspws, Nfreqs, Npols) & v1.0\\[1.75em] \hline
    \texttt{Data/flags} & Array of numpy bool, shape (Nblts, Nfreqs, Npols) & Array of numpy bool, shape (Nblts, Nspws, Nfreqs, Npols) & v1.0\\[1.75em] \hline
    \texttt{Data/nsamples} & Array of float, shape (Nblts, Nfreqs, Npols) & Array of float, shape (Nblts, Nspws, Nfreqs, Npols) & v1.0\\
  \end{tabular}
\end{center}
