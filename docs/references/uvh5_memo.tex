...
catalog names. The next level keys must include:
	...
	\item \textbf{cat\_type}: \textit{string} One of four allowed values:
  	...
  	\item \textbf{cat\_lon}: \textit{float or ndarray of float (ephem only)}
	  The longitudinal coordinate of the phase center. This is commonly RA,
	  but can also be galactic longitude is azimuth for driftscan phase centers.
	  For ephem phase centers this is an array of length Npts (the number of
	  ephemeris data points).
  	\item \textbf{cat\_lat}: \textit{float or ndarray of float (ephem only)}
	  The latitudinal coordinate of the phase center. This is commonly Dec,
	  but is elevation (altitude) for driftscan phase centers. For ephem phase centers
	  this is an array of length Npts (the number of ephemeris data points).
	...
	\item \textbf{cat\_epoch}: \textit{float} The epoch in years for the phase
	  ...
	  is proper motion (specified in the \cat_\pm_\ra and \cat_\pm_\dec keys).
