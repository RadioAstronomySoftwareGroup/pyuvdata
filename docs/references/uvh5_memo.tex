...
\item \textbf{uvw\_array}: \textit{float} An array of the uvw-coordinates
  corresponding to each observation in the data. This is a two-dimensional array 
  of size (Nblts, 3). Baselines are specified as from \textit{ant\_1\_array} to 
  \textit{ant\_2\_array}, inferring the position of antenna 2 minus the position of 
  antenna 1. Units are in meters. (\textit{uvw\_array})
...
the specification of the baselines, antenna $i$ is indexed as per
\textit{ant\_1\_array} and $j$ is indexed as per \textit{ant\_2\_array}. Note
that if a file is generated with the opposite convention, it is usually
sufficient to multiply $uvw$ coordinates by $-1$ to generate a self-consistent 
dataset, as well as conjugating the data in the \verb+data_array+.
