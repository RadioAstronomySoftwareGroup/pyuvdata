...
baseline-times are phased to which phase center via the phase\_center\_id\_array.
...
phase\_center\_catalog under the cat\_frame dataset. This is a one dimensional array
...
types of phase centers, the  \textbf{phase\_center\_catalog},
\textbf{phase\_center\_id\_array}, \textbf{phase\_center\_app\_ra},
\textbf{phase\_center\_app\_dec}, and \textbf{phase\_center\_frame\_pa}
(described in \ref{sec:req_params}) were added and the following header
items (found in versions less than 1.1) were removed:
...
For a brief period of time, while the version number was still 1.0, the new phase attributes
were written to files along with the header items listed above. During this time, the
\textbf{phase\_center\_catalog} was written as a python dict converted to a JSON-formatted
string. This intermediate file format was undocumented and not widely used, but it is possible
some files like this exist ``in the wild".
