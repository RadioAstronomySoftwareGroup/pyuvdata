...
def _select_blt_preprocess(
    *,
    ...
    if select_antenna_nums is not None:
        select_antenna_nums = uvutils._get_iterable(select_antenna_nums)
    ...
    if np.count_nonzero([have_times, have_time_range, have_lsts, have_lst_range]) > 1:
        raise ValueError(
    ...
    if times is not None:
        ...
        for jd in times:
            if np.any(np.isclose(time_array, jd, rtol=time_tols[0], atol=time_tols[1])):
                time_blt_inds = np.append(
                    time_blt_inds,
                    np.where(
                        np.isclose(time_array, jd, rtol=time_tols[0], atol=time_tols[1])
                    )[0],
                )
            else:
                raise ValueError(
    ...
    if time_range is not None:
        ...
        time_blt_inds = np.nonzero(
            (time_array <= time_range[1]) & (time_array >= time_range[0])
        )[0]
    ...
    if lsts is not None:
        ...
        time_blt_inds = np.zeros(0, dtype=np.int64)
        for lst in lsts:
            if np.any(np.isclose(lst_array, lst, rtol=lst_tols[0], atol=lst_tols[1])):
                time_blt_inds = np.append(
                    time_blt_inds,
                    np.where(
                        np.isclose(lst_array, lst, rtol=lst_tols[0], atol=lst_tols[1])
                    )[0],
                )
            else:
    ...
    if lst_range is not None:
        ...
        if lst_range[1] < lst_range[0]:
            ...
            time_blt_inds1 = np.nonzero(
                (lst_array <= lst_range_1[1]) & (lst_array >= lst_range_1[0])
            )[0]
            time_blt_inds2 = np.nonzero(
                (lst_array <= lst_range_2[1]) & (lst_array >= lst_range_2[0])
            )[0]
            time_blt_inds = np.union1d(time_blt_inds1, time_blt_inds2)
        else:
            time_blt_inds = np.nonzero(
                (lst_array <= lst_range[1]) & (lst_array >= lst_range[0])
            )[0]
        if time_blt_inds.size == 0:
            raise ValueError(
...
class UVData(UVBase):
    ...
    def _select_preprocess(
        ...
        if bls is not None:
            ...
            if any(len(item) == 3 for item in bls):
                if polarizations is not None:
                    raise ValueError(
                        "Cannot provide any length-3 tuples and also specify "
                        "polarizations."
                    )
                ...
                for bl_i, bl in enumerate(bls):
                    ...
                    if not isinstance(bl[2], str):
                        raise ValueError(
                            "The third element in a bl tuple must be a "
                            "polarization string"
                        )
                    ...
                    wh1 = np.where(
                        np.logical_and(
                            self.ant_1_array == bl[0], self.ant_2_array == bl[1]
                        )
                    )[0]
                    if len(wh1) > 0:
        ...
        blt_inds, blt_selections = _select_blt_preprocess(
