...
def test_frequency_average_flagging_full_and_partial(uvdata_data):
    """Test averaging in frequency with flagging all of one and only one of another sample averaged."""
    # check that there's no flagging
    assert np.nonzero(uvdata_data.uv_object.flag_array)[0].size == 0
    ...
    assert np.nonzero(uvdata_data.uv_object.flag_array)[0].size == uvdata_data.uv_object.Npols * 3
