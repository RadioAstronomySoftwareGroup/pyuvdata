...
class FHDCal(UVCal):
    ...
    def read_fhd_cal(self, cal_file, obs_file, settings_file=None, raw=True,
        ...
        self.telescope_name = uvutils.bytes_to_str(obs_data['instrument'][0])

        self.Nants_data = int(cal_data['n_tile'][0])
        self.Nants_telescope = int(cal_data['n_tile'][0])
        self.antenna_names = np.array([uvutils.bytes_to_str(n) for n in cal_data['tile_names'][0].tolist()])
        ...
        self.sky_catalog = uvutils.bytes_to_str(cal_data['skymodel'][0]['catalog_name'][0])
        self.ref_antenna_name = uvutils.bytes_to_str(cal_data['ref_antenna_name'][0])
        ...
        if isinstance(diffuse_model, six.binary_type):
            diffuse_model = uvutils.bytes_to_str(diffuse_model)
        if diffuse_model == '':
            diffuse_model = None
        else:
        ...
        self.git_hash_cal = uvutils.bytes_to_str(obs_data['code_version'][0])
