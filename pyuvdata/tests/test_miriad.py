...
def test_ReadMiriadWriteUVFits():
    ...
    assert str(cm.value).startswith('The data are in drift mode. Set force_phase')
    ...
    assert str(cm.value).startswith('Required attribute')
    ...
    with pytest.raises(ValueError) as cm:
        uvtest.checkWarnings(miriad_uv.read, func_args=[miriad_file],
                             func_kwargs={"phase_type": 'phased'},
                             known_warning='miriad')
        # miriad_uv.read(miriad_file, phase_type='phased')
    assert str(cm.value).startswith('phase_type is "phased" but the RA values are varying')

    with pytest.raises(ValueError) as cm:
        uvtest.checkWarnings(miriad_uv.read, func_args=[miriad_file],
                             func_kwargs={"phase_type": 'foo'},
                             known_warning='miriad')
        # miriad_uv.read(miriad_file, phase_type='foo')
    assert str(cm.value).startswith('The phase_type was not recognized.')
...
def test_singletimeselect_drift():
    ...
    miriad_file = os.path.join(DATA_PATH, 'zen.2456865.60537.xy.uvcRREAA')
    testfile = os.path.join(DATA_PATH, 'test/outtest_miriad.uv')
    uvtest.checkWarnings(uv_in.read, func_args=[miriad_file],
                         known_warning='miriad')
    # uv_in.read(miriad_file)
...
def test_readWriteReadMiriad():
    ...
    uvtest.checkWarnings(uv_in.read, func_args=[testfile],
                         known_warning='miriad')
    # uv_in.read(testfile)
    ...
    uvtest.checkWarnings(uv_in._load_telescope_coords, func_args=[uv],
                         known_warning='miriad')
    ...
    uv_in = Miriad()
    uv = aipy_extracts.UV(testfile)
    uvtest.checkWarnings(uv_in._load_antpos, func_args=[uv],
                         known_warning='miriad')
...
def test_readWriteReadMiriad_partial_bls(select_kwargs):
    ...
    full = UVData()
    uvtest.checkWarnings(full.read, func_args=[testfile],
                         known_warning='miriad')
    # full.read(testfile)
...
def test_multi_files():
    ...
    del(uv1)
    uv1 = UVData()
    uvtest.checkWarnings(uv1.read_miriad, [[testfile1, testfile2]],
                         nwarnings=2,
                         message='Telescope EVLA is not')
    ...
    del(uv1)
    uv1 = UVData()
    uvtest.checkWarnings(uv1.read, func_args=[[testfile1, testfile2]],
                         func_kwargs={"axis": 'freq'}, nwarnings=2,
                         message='Telescope EVLA is not')
